...
function updateBars(house) {
    ...
    if (house == "top") {
        var filteredChar = characters.filter(function(value,index) {
            return (characters[index].power_ranking > 0)
        })
    } else {
        var filteredChar = characters.filter(function(value, index) {
            return (characters[index].house == house)
        })
    }
    filteredChar.sort(function(a,b) {
        return b.probability_of_survival - a.probability_of_survival
    })
    renderBars(filteredChar)
}