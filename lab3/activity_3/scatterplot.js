...
d3.csv('baseball_hr_leaders.csv').then(function(dataset) {
    var group = svg.selectAll('g')
        .data(dataset)
        .enter()
        .append('g') //append a <g> element for each data case
        .attr('transform', function(d) { return 'translate(' + scaleYear(d.year) + ',' + scaleHomeruns(d.homeruns) + ')' })
        //translate the group based on the year and homerunes data attributes   
        group.append('circle') //append circle
        .attr('r', '2px')

        group.append('text') //append text
        .attr('transform', 'translate(0,-5)') //translate text to have it shown above the circle
        .attr('class', 'body') //assign rule
